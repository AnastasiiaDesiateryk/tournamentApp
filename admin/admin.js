...
const renderManagedTournamentsTable = () => {
  const tournamentManageTable = document.getElementById(
    "manage-tournaments-table-body"
  );
...
const renderTournamentMatchesTable = () => {
  const tournamentMatchesTable = document.getElementById(
    "tournament-matches-table-body"
  );
      ...
      for (let j = 0; j < tournamentItem.matches[i].length; j++) {
        tournamentMatchesTableBody += `
            <tr>
              ...
              <td>
              ${getUsernameById(
                tournamentItem.matches[i][j][0]
              )} vs ${getUsernameById(tournamentItem.matches[i][j][1])}
              </td>
              <td>
                ${
                  user.role == "admin"
                    ? `<button type="button" class="btn btn-secondary">Set Score</button>`
                    : ""
                }
              </td>
            </tr>
...
const getNumberOfActiveTournaments = () => {
  ...
  const numberOfActiveTournamentsTag = document.getElementById(
...
const getNumberOfParticipants = () => {
  ...
  const numberOfPaticipantsTag = document.getElementById(
